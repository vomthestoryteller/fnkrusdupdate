...
function renderTimeline(data, filter, containerId = 'timeline-container') {
    ...
    // Filter data if a specific product filter is applied (and not 'all')
    // If the data is already specific (e.g. loaded from nuke.json), filtering by 'nuke' is redundant but harmless.
    const filteredData = filter === 'all' 
        ? data 
        : data.filter(item => item.product === filter);
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
        ...
        // Check if the button corresponds to the current filter
        if (filter === 'all') {
             if (btn.classList.contains('filter-btn-all')) {
                btn.classList.add('active');
             }
        } else {
            // For individual product pages, we might want to highlight the specific product button
            // regardless of whether we are "filtering" or just showing that product's data.
